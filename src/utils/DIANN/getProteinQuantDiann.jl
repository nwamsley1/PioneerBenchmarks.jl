...
function getProteinQuantDIANN(
    ...
    [println(name) for name in names(diann_df)]
    proteinGroupCols = [:Run,:Condition,:Experiment,:Species,:StrippedSequence,:PrecursorQuantity,:QValue,:LibQValue,:ProteinGroup,:ProteinIds,
    :ProteinNames,:PGMaxLFQ,:PGQValue,:LibPGQValue]
    ...
    filter!(x->x.LibQValue<=max_precursor_q_val, diann_pg)
    filter!(x->!iszero(x.PrecursorQuantity), diann_pg)
    select!(diann_pg, Not([:PrecursorQuantity,:QValue,:LibQValue]))
    ...
    filter!(x->x.LibPGQValue<=max_pg_q_val, diann_pg)
...
function getMMCCProteinQuantDIANN(
    ...
    proteinGroupCols = [:Run,:Condition,:StrippedSequence,:PrecursorQuantity,:QValue,:ProteinGroup,:ProteinIds,:ProteinNames,:PGMaxLFQ,:PGQValue]
    diann_pg = copy(diann_df[!,proteinGroupCols])
    ###########
    #Filter Rows 
    filterRowsOnSequenceLength!(
